...
| **"Canonicalised" Red Warning in SEO Extensions (Trailing Slash Mismatch)** | Mismatch between the visited URL format (with/without slash) and the canonical tag causes SEO extensions (e.g. Detailed SEO) to flag "Canonicalised" instead of "Self-Canonical". | Google Search Central strictly enforces consistency across Sitemap, Canonical, and Internal links. For AIA: Course pages (`COURSE_ROUTES`: `/cfe-curriculum`, `/cia-curriculum`, `/cia-challenge-curriculum`, `/cams`, `/cisa`) are indexed **WITHOUT** a trailing slash; all other pages (Blogs, Free Resources, About, etc.) are indexed **WITH** a trailing slash. Standardize all internal links, sitemaps, `.htaccess` redirects, and canonical tags to match this exact format to ensure 100% "Self-Canonical" green status. |
